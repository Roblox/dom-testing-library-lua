...
return function()
	...
	Array.forEach(eventTypes, function(ref)
		...
		describe(("%s Events"):format(type_), function()
			Array.forEach(events, function(event)
				...
				it(("fires %s"):format(event.name), function()
	...
	it("assigns target properties", function()
		...
		jestExpect(node.Text).toBe(value)
		-- ROBLOX deviation END
	end)

	it("assigns selection-related target properties", function()
		local node = Instance.new("TextBox")
		local spy = jest.fn()
		local value = "ab"
		...
		jestExpect(node.Text).toBe(value)
		-- ROBLOX deviation END
		jestExpect(node.SelectionStart).toBe(selectionStart)
		jestExpect(node.CursorPosition).toBe(selectionEnd)
	end)

	it("assigning a value to a target that cannot have a value throws an error", function()
		local node = Instance.new("Frame")
		jestExpect(function()
	...
	-- 	jestExpect(node.files).toEqual({ file })
	-- end)
	-- ROBLOX deviation END
	...
	-- 	jestExpect(clipboardData.items[
	-- 		1 --[[ ROBLOX adaptation: added 1 to array index ]]
	-- 	].file:getAsFile()).toBeNull()
	-- 	jestExpect(clipboardData:getData("text")).toBe("example")
	-- end)
	-- ROBLOX deviation END
	...
	-- 	jestExpect(messageSpy).toHaveBeenCalledTimes(1)
	...
	-- 	jestExpect(popStateSpy).toHaveBeenCalledTimes(1)
	...
	-- 	jestExpect(clickSpy).toHaveBeenCalledTimes(1)
	-- 	window:removeEventListener("click", clickSpy)
	-- end)
	-- ROBLOX deviation END

	it("throws a useful error message when firing events on non-existent nodes", function()
		jestExpect(function()
	...
	it("throws a useful error message when firing events on non-existent nodes (shortcut)", function()
		jestExpect(function()
	...
	it("throws a useful error message when firing non-events", function()
	...
	it("fires events on document", function()
		local div = Instance.new("Frame")
		...
		jestExpect(keyDownSpy).toHaveBeenCalledTimes(1)
		connection:Disconnect()
	end)
	-- ROBLOX deviation END
	...
	-- 	jestExpect(handler).toHaveBeenCalledWith(event)
	-- end)
	-- ROBLOX deviation END
end
